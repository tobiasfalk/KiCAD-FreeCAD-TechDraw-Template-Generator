...
bool TemplateGenKiCAD_7::getLOGO() const
{
    return LOGO;
}

void TemplateGenKiCAD_7::setLOGO(bool newLOGO)
{
    LOGO = newLOGO;
    finisheD.logO = true;
}

const QString &TemplateGenKiCAD_7::getLOGODIR() const
{
    return LOGODIR;
}

void TemplateGenKiCAD_7::setLOGODIR(const QString &newLOGODIR)
{
    LOGODIR = newLOGODIR;
    finisheD.logodiR = true;
}

const QString &TemplateGenKiCAD_7::getSHEETNAME() const
{
    return SHEETNAME;
}

void TemplateGenKiCAD_7::setSHEETNAME(const QString &newSHEETNAME)
{
    SHEETNAME = newSHEETNAME;
    finisheD.sheetnamE = true;
}

bool TemplateGenKiCAD_7::init()
{
...
QString TemplateGenKiCAD_7::createFileName()
{
    QString ret =  DIR + "/" + PAGESIZE.sizeString;
...
void TemplateGenKiCAD_7::drawLine(Coordinate start, Coordinate end, double lineWidth)
{
...
void TemplateGenKiCAD_7::drawRect(Coordinate start, Coordinate end, double lineWidth)
{
...
void TemplateGenKiCAD_7::drawPoly(Coordinate position, double rotaition, QList<Coordinate> points, double lineWidth)
{
...
qint64 TemplateGenKiCAD_7::drawText(Coordinate position, QString text, double textSize, TextHeightAnchor textHeightAnchor, TextWidthAnchor textWidthAnchor, double lineWidth, qint64 line, QString font)
{
    if(line > -1)
    {
        text = text.replace("&{L}", NumToABC(line).at(0));
    }
    else
    {
        text = text.replace("&{L}", "");
...
qint64 TemplateGenKiCAD_7::drawText(Coordinate position, QStringList text, double textSize, TextHeightAnchor textHeightAnchor, TextWidthAnchor textWidthAnchor, double lineWidth, qint64 index)
{
    qint64 ret = 0;
    if(text.length() == 1)
    {
    ...
    else if(text.length() > 1)
    {
        ...
        case TextHeightAnchor::Top:
            for(int i = 0; i < text.length(); i++)
            {
                ...
                ret += drawText(Coordinate{position.X, height}, txt.replace("&{N}", QString::number(index + 1)), textSize, TextHeightAnchor::Top, textWidthAnchor, lineWidth, i + 1);
            }
            break;
...
void TemplateGenKiCAD_7::drawVerFoldLine(double x, double depth)
{
...
void TemplateGenKiCAD_7::drawHorFoldLine(double y, double depth)
{
...
void TemplateGenKiCAD_7::drawTrimmingMarksISO5457()
{
...
void TemplateGenKiCAD_7::drawDrawingBorderISO5457()
{
    ...
    // Top and Bottom Framegrid
    {
        ...
        foreach(double pos, indexListLeft)
        {
            ...
            drawText(Coordinate{pos, 8}, QString::number(index), 3.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.35);
            if(PAGESIZE.height >= 297 && PAGESIZE.width >= 297)
            {
                drawText(Coordinate{pos, PAGESIZE.height - 7}, QString::number(index), 3.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.35);
            }
            index++;
        }
        ...
        //Middle Text
        // Left
        drawText(Coordinate{halfPageWidth - 25, 8}, QString::number(index), 3.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.35);
        if(PAGESIZE.height >= 297 && PAGESIZE.width >= 297)
        {
            drawText(Coordinate{halfPageWidth - 25, PAGESIZE.height - 7}, QString::number(index), 3.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.35);
        }
        index++;
        // Right
        drawText(Coordinate{halfPageWidth + 25, 8}, QString::number(index), 3.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.35);
        ...
        while(lineX < PAGESIZE.width - spaceRight)
        {
            // Top
            drawLine(Coordinate{lineX, 5}, Coordinate{lineX, 10}, 0.35);
            drawText(Coordinate{lineX + 25, 8}, QString::number(index), 3.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.35);
            //Bottom
            ...
            {
                drawText(Coordinate{lineX + 25, PAGESIZE.height - 7}, PAGESIZE.sizeString, 3.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.35);
            }
            else if(lineX >= PAGESIZE.width - 60)
            {
                drawText(Coordinate{lastBottomLine + (PAGESIZE.width - lastBottomLine) / 2, PAGESIZE.height - 7}, PAGESIZE.sizeString, 3.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.35);
            }
            else if(PAGESIZE.height >= 297  && lineX < PAGESIZE.width - spaceRightBottom - 50)
            {
                drawText(Coordinate{lineX + 25, PAGESIZE.height - 7}, QString::number(index), 3.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.35);
            }
            lineX += 50;
            index++;
        }
    ...
    // Left and Right Framgrid
    {
        ...
        foreach(double pos, indexListTop)
        {
            ...
            {
                drawText(Coordinate{PAGESIZE.width - 7, pos}, NumToABC(index), 3.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.35);
            }
            index++;
        }
        // Middle
        ...
        if(PAGESIZE.width >= 297 && PAGESIZE.height >= 297)
        {
            drawText(Coordinate{PAGESIZE.width - 7, halfPageHeight - 25}, NumToABC(index), 3.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.35);
        }
        index++;
        drawText(Coordinate{17.5, halfPageHeight + 25}, NumToABC(index), 3.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.35);
        if(PAGESIZE.width >= 297 && PAGESIZE.height >= 297)
        ...
        while(lineY < PAGESIZE.height - spaceBottom)
        {
            ...
            {
                drawText(Coordinate{PAGESIZE.width - 7, lineY + 25}, NumToABC(index), 3.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.35);
            }
            else if(PAGESIZE.width >= 297  && lineY > PAGESIZE.height - 60 && PAGESIZE.height >= 297)
            {
                drawText(Coordinate{PAGESIZE.width - 7, textY}, NumToABC(index), 3.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.35);
            }
            lineY += 50;
            index++;
        }
...
void TemplateGenKiCAD_7::drawTitelblockISO7200()
{
    // Frame
    ...
    for(qint64 i = 0; i < NUMOPTLINES; i++)
    {
        if(NUMOPTLINES == 3 && i == 0)
        {
            ...
            drawText(Coordinate{lastLineLeft.X + 1.5, lastLineLeft.Y + 1.5 + 4.75}, TITELBLOCKFIELDS["opt1"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Left, 0.25);
            drawText(Coordinate{lastLineLeft.X + 1.5 + 80, lastLineLeft.Y + 1.5 + 4.75}, TITELBLOCKFIELDS["opt2"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Left, 0.25);
            drawText(Coordinate{lastLineLeft.X + 1.5 + 140, lastLineLeft.Y + 1.5 + 4.75}, TITELBLOCKFIELDS["opt3"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Left, 0.25);
        }
        else if((NUMOPTLINES == 3 && i == 1) || (NUMOPTLINES == 2 && i == 0))
        {
            ...
            drawText(Coordinate{lastLineLeft.X + 1.5, lastLineLeft.Y + 1.5 + 4.75}, TITELBLOCKFIELDS["opt4"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Left, 0.25);
            drawText(Coordinate{lastLineLeft.X + 1.5 + 80, lastLineLeft.Y + 1.5 + 4.75}, TITELBLOCKFIELDS["opt5"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Left, 0.25);
            drawText(Coordinate{lastLineLeft.X + 1.5 + 140, lastLineLeft.Y + 1.5 + 4.75}, TITELBLOCKFIELDS["opt6"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Left, 0.25);
        }
        else if((NUMOPTLINES == 3 && i == 2) || (NUMOPTLINES == 2 && i == 1) || (NUMOPTLINES == 1 && i == 0))
        {
            ...
            drawText(Coordinate{lastLineLeft.X + 1.5, lastLineLeft.Y + 1.5 + 4.75}, TITELBLOCKFIELDS["opt7"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Left, 0.25);
            drawText(Coordinate{lastLineLeft.X + 1.5 + 80, lastLineLeft.Y + 1.5 + 4.75}, TITELBLOCKFIELDS["opt8"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Left, 0.25);
            drawText(Coordinate{lastLineLeft.X + 1.5 + 140, lastLineLeft.Y + 1.5 + 4.75}, TITELBLOCKFIELDS["opt9"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Left, 0.25);
        }
    ...
    drawText(Coordinate{lastLineLeft.X + 1.5, lastLineLeft.Y + 1.5 + 4.75}, TITELBLOCKFIELDS["opt10"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Left, 0.25);
    drawText(Coordinate{lastLineLeft.X + 1.5 + 80, lastLineLeft.Y + 1.5 + 4.75}, TITELBLOCKFIELDS["opt11"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Left, 0.25);
    drawText(Coordinate{lastLineLeft.X + 1.5 + 140, lastLineLeft.Y + 1.5 + 4.75}, TITELBLOCKFIELDS["opt12"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Left, 0.25);
    lastLineLeft = Coordinate{lastLineLeft.X, lastLineLeft.Y + 11};
    drawLine(lastLineLeft, Coordinate{PAGESIZE.width - 10, lastLineLeft.Y}, 0.35);
    ...
    drawText(Coordinate{lastLineLeft.X + 1.5, lastLineLeft.Y + 1.5 + 4.75}, TITELBLOCKFIELDS["opt13"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Left, 0.25);
    drawText(Coordinate{lastLineLeft.X + 1.5 + 80, lastLineLeft.Y + 1.5 + 4.75}, TITELBLOCKFIELDS["opt14"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Left, 0.25);
    drawText(Coordinate{lastLineLeft.X + 1.5 + 140, lastLineLeft.Y + 1.5 + 4.75}, TITELBLOCKFIELDS["opt15"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Left, 0.25);
    lastLineLeft = Coordinate{lastLineLeft.X, lastLineLeft.Y + 11};
    drawLine(lastLineLeft, Coordinate{PAGESIZE.width - 10, lastLineLeft.Y}, 0.35);
    ...
    drawText(Coordinate{lastLineLeft.X + 1.5, lastLineLeft.Y + 1.5 + 4.75}, TITELBLOCKFIELDS["opt19"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Left, 0.25);
    drawText(Coordinate{lastLineLeft.X + 1.5 + 12,  lastLineLeft.Y + 1.5 + 4.75}, TITELBLOCKFIELDS["opt20"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Left, 0.18);
    lastLineLeft = Coordinate{lastLineLeft.X, lastLineLeft.Y + 11};
    drawLine(lastLineLeft, Coordinate{PAGESIZE.width - 10, lastLineLeft.Y}, 0.35);
    ...
    drawText(Coordinate{lastLineLeft.X + 1.5, lastLineLeft.Y + 1.5 + 4.75}, TITELBLOCKFIELDS["opt21"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Left, 0.25);
    drawText(Coordinate{lastLineLeft.X + 1.5 + 12,  lastLineLeft.Y + 1.5 + 4.75}, TITELBLOCKFIELDS["opt22"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Left, 0.25);

    topLeft = Coordinate{PAGESIZE.width - 190, PAGESIZE.height - 10 - 11 * (5 + NUMOPTLINES)};
}

void TemplateGenKiCAD_7::drawRevHistoryASME_Y14_35_Width180()
{
    ...
    for(int i = 0; i < NUMREVHISTORY; i++)
    {
        ...
        drawText(Coordinate{TOPRIGHTDRAWINGCORNER.X - 172.5, TOPRIGHTDRAWINGCORNER.Y + 9.5 + 13.5 * i}, REVHISTORYFIELDS["opt1"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Center, 0.25, i);
        drawText(Coordinate{TOPRIGHTDRAWINGCORNER.X - 155, TOPRIGHTDRAWINGCORNER.Y + 9.5 + 13.5 * i}, REVHISTORYFIELDS["opt2"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Center, 0.25, i);
        drawText(Coordinate{TOPRIGHTDRAWINGCORNER.X - 97.5, TOPRIGHTDRAWINGCORNER.Y + 9.5 + 13.5 * i}, REVHISTORYFIELDS["opt3"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Center, 0.25, i);
        drawText(Coordinate{TOPRIGHTDRAWINGCORNER.X - 40, TOPRIGHTDRAWINGCORNER.Y + 9.5 + 13.5 * i}, REVHISTORYFIELDS["opt4"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Center, 0.25, i);
        drawText(Coordinate{TOPRIGHTDRAWINGCORNER.X - 15, TOPRIGHTDRAWINGCORNER.Y + 9.5 + 13.5 * i}, REVHISTORYFIELDS["opt5"].ValueKiCAD6, 2.5, TextHeightAnchor::Top, TextWidthAnchor::Center, 0.25, i);
    }
}

void TemplateGenKiCAD_7::drawFoldLines(double depth)
{
    switch (PAGESTYLE)
    {
    case PageStyle::ISO5457_ISO7200:
...
void TemplateGenKiCAD_7::drawSmallPartsList()
{
    ...
    for(int i = 1; i <= NUMPARTSSMALLPARTSLIST; i++)
    {
        ...
        {
            opt1Val.append(SMALLPARTSLISTFIELDS["opt1"].ValueKiCAD6.at(0));
            opt2Val.append(SMALLPARTSLISTFIELDS["opt2"].ValueKiCAD6.at(0));
            opt3Val.append(SMALLPARTSLISTFIELDS["opt3"].ValueKiCAD6.at(0));
            opt4Val.append(SMALLPARTSLISTFIELDS["opt4"].ValueKiCAD6.at(0));
            opt5Val.append(SMALLPARTSLISTFIELDS["opt5"].ValueKiCAD6.at(0));
        }
        drawText(Coordinate{topLeft.X + (width * (double(1)/18)) / 2, topLeft.Y + (fildHeight * (i - 1)) + fildHeight / 2 + 0.5}, opt1Val, 2.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.25, i - 1);
        drawText(Coordinate{topLeft.X + width * (double(1)/18) + (width * (double(3)/36)) / 2, topLeft.Y + (fildHeight * (i - 1)) + fildHeight / 2 + 0.5}, opt2Val, 2.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.25, i - 1);
        drawText(Coordinate{topLeft.X + width * (double(5)/36) + (width * (double(2)/36)) / 2, topLeft.Y + (fildHeight * (i - 1)) + fildHeight / 2 + 0.5}, opt3Val, 2.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.25, i - 1);
        drawText(Coordinate{topLeft.X + width * (double(7)/36) + (width * (double(14)/36)) / 2, topLeft.Y + (fildHeight * (i - 1)) + fildHeight / 2 + 0.5}, opt4Val, 2.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.25, i - 1);
        drawText(Coordinate{topLeft.X + width * (double(21)/36) + (width * (double(15)/36)) / 2, topLeft.Y + (fildHeight * (i - 1)) + fildHeight / 2 + 0.5}, opt5Val, 2.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.25, i - 1);
...
void TemplateGenKiCAD_7::drawFullSheetPartsList()
{
    ...
    while(indexTop <= bottom)
    {
        ...
        {
            opt1Val.append(FULLSHEETPARTSLISTFIELDS["opt1"].ValueKiCAD6.at(0));
            opt2Val.append(FULLSHEETPARTSLISTFIELDS["opt2"].ValueKiCAD6.at(0));
            opt3Val.append(FULLSHEETPARTSLISTFIELDS["opt3"].ValueKiCAD6.at(0));
            opt4Val.append(FULLSHEETPARTSLISTFIELDS["opt4"].ValueKiCAD6.at(0));
            opt5Val.append(FULLSHEETPARTSLISTFIELDS["opt5"].ValueKiCAD6.at(0));
            opt6Val.append(FULLSHEETPARTSLISTFIELDS["opt6"].ValueKiCAD6.at(0));
        }
        drawText(Coordinate{left + (width * (2/double(36))) / 2, indexTop - fieldHeight/2}, opt1Val, 2.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.25, i);
        drawText(Coordinate{left + width * (2/double(36)) + (width * (3/double(36))) / 2, indexTop - fieldHeight/2}, opt2Val, 2.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.25, i);
        drawText(Coordinate{left + width * (5/double(36)) + (width * (2/double(36))) / 2, indexTop - fieldHeight/2}, opt3Val, 2.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.25, i);
        drawText(Coordinate{left + width * (7/double(36)) + (width * (14/double(36))) / 2, indexTop - fieldHeight/2}, opt4Val, 2.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.25, i);
        drawText(Coordinate{left + width * (21/double(36)) + (width * (8/double(36))) / 2, indexTop - fieldHeight/2}, opt5Val, 2.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.25, i);
        drawText(Coordinate{left + width * (29/double(36)) + (width * (7/double(36))) / 2, indexTop - fieldHeight/2}, opt6Val, 2.5, TextHeightAnchor::Middle, TextWidthAnchor::Center, 0.25, i);
...
void TemplateGenKiCAD_7::drawLogoTitelblockISO7200()
{
    // Load SVG
...
void TemplateGenKiCAD_7::drawISO5457_ISO7200()
{
    if(TRIMMINGMARKS)
    {
...
TemplateGenKiCAD_7::TemplateGenKiCAD_7(QObject *parent) : QObject(parent)
{
}
...
const QString &TemplateGenKiCAD_7::getDIR() const
{
    return DIR;
}

void TemplateGenKiCAD_7::setDIR(const QString &newDIR)
{
    finisheD.diR = true;
    DIR = newDIR;
}

const PageSize &TemplateGenKiCAD_7::getPAGESIZE() const
{
    return PAGESIZE;
}

void TemplateGenKiCAD_7::setPAGESIZE(const PageSize &newPAGESIZE)
{
    finisheD.pagesizeE = true;
    PAGESIZE = newPAGESIZE;
}

PageStyle TemplateGenKiCAD_7::getPAGESTYLE() const
{
    return PAGESTYLE;
}

void TemplateGenKiCAD_7::setPAGESTYLE(PageStyle newPAGESTYLE)
{
    finisheD.pagestylE = true;
    PAGESTYLE = newPAGESTYLE;
}

qint64 TemplateGenKiCAD_7::getNUMOPTLINES() const
{
    return NUMOPTLINES;
}

void TemplateGenKiCAD_7::setNUMOPTLINES(qint64 newNUMOPTLINES)
{
    NUMOPTLINES = newNUMOPTLINES;
    finisheD.numoptlineS = true;
}

const QMap<QString, TitelblockField> &TemplateGenKiCAD_7::getTITELBLOCKFIELDS() const
{
    return TITELBLOCKFIELDS;
}

void TemplateGenKiCAD_7::setTITELBLOCKFIELDS(const QMap<QString, TitelblockField> &newTITELBLOCKFIELDS)
{
    TITELBLOCKFIELDS = newTITELBLOCKFIELDS;
    finisheD.titelblockfieldS = true;
}

bool TemplateGenKiCAD_7::getTRIMMINGMARKS() const
{
    return TRIMMINGMARKS;
}

void TemplateGenKiCAD_7::setTRIMMINGMARKS(bool newTRIMMINGMARKS)
{
    TRIMMINGMARKS = newTRIMMINGMARKS;
    finisheD.trimmingmarkS = true;
}

RevHistoryStyle TemplateGenKiCAD_7::getREVHISTORYSTYLE() const
{
    return REVHISTORYSTYLE;
}

void TemplateGenKiCAD_7::setREVHISTORYSTYLE(RevHistoryStyle newREVHISTORYSTYLE)
{
    REVHISTORYSTYLE = newREVHISTORYSTYLE;
    finisheD.revhistorystylE = true;
}

const QMap<QString, TitelblockField> &TemplateGenKiCAD_7::getREVHISTORYFIELDS() const
{
    return REVHISTORYFIELDS;
}

void TemplateGenKiCAD_7::setREVHISTORYFIELDS(const QMap<QString, TitelblockField> &newREVHISTORYFIELDS)
{
    REVHISTORYFIELDS = newREVHISTORYFIELDS;
    finisheD.revhistoryfieldS = true;
}

qint64 TemplateGenKiCAD_7::getNUMREVHISTORY() const
{
    return NUMREVHISTORY;
}

void TemplateGenKiCAD_7::setNUMREVHISTORY(qint64 newNUMREVHISTORY)
{
    NUMREVHISTORY = newNUMREVHISTORY;
    finisheD.numrevhistorY = true;
}

bool TemplateGenKiCAD_7::getREVHISTORY() const
{
    return REVHISTORY;
}

void TemplateGenKiCAD_7::setREVHISTORY(bool newREVHISTORY)
{
    REVHISTORY = newREVHISTORY;
    finisheD.revhistorY = true;
}

bool TemplateGenKiCAD_7::getFOLDLINES() const
{
    return FOLDLINES;
}

void TemplateGenKiCAD_7::setFOLDLINES(bool newFOLDLINES)
{
    FOLDLINES = newFOLDLINES;
    finisheD.foldlineS = true;
}

const PageSize &TemplateGenKiCAD_7::getFOLDLINETARGET() const
{
    return FOLDLINETARGET;
}

void TemplateGenKiCAD_7::setFOLDLINETARGET(const PageSize &newFOLDLINETARGET)
{
    FOLDLINETARGET = newFOLDLINETARGET;
    finisheD.foldlinestargeT = true;
}

quint64 TemplateGenKiCAD_7::getNUMPARTSSMALLPARTSLIST() const
{
    return NUMPARTSSMALLPARTSLIST;
}

void TemplateGenKiCAD_7::setNUMPARTSSMALLPARTSLIST(quint64 newNUMPARTSSMALLPARTSLIST)
{
    NUMPARTSSMALLPARTSLIST = newNUMPARTSSMALLPARTSLIST;
    finisheD.numpartssmallpartslisT = true;
}

bool TemplateGenKiCAD_7::getSMALLPARTSLIST() const
{
    return SMALLPARTSLIST;
}

void TemplateGenKiCAD_7::setSMALLPARTSLIST(bool newSMALLPARTSLIST)
{
    SMALLPARTSLIST = newSMALLPARTSLIST;
    finisheD.smallpartslisT = true;
}

quint64 TemplateGenKiCAD_7::getNUMLINESMALLPARTSLIST() const
{
    return NUMLINESMALLPARTSLIST;
}

void TemplateGenKiCAD_7::setNUMLINESMALLPARTSLIST(quint64 newNUMLINESMALLPARTSLIST)
{
    NUMLINESMALLPARTSLIST = newNUMLINESMALLPARTSLIST;
    finisheD.numlinessmallpartslisT = true;
}

const QMap<QString, TitelblockField> &TemplateGenKiCAD_7::getSMALLPARTSLISTFIELDS() const
{
    return SMALLPARTSLISTFIELDS;
}

void TemplateGenKiCAD_7::setSMALLPARTSLISTFIELDS(const QMap<QString, TitelblockField> &newSMALLPARTSLISTFIELDS)
{
    SMALLPARTSLISTFIELDS = newSMALLPARTSLISTFIELDS;
    finisheD.smallpartslistfieldS = true;
}

bool TemplateGenKiCAD_7::getFULLSHEETPARTSLIST() const
{
    return FULLSHEETPARTSLIST;
}

void TemplateGenKiCAD_7::setFULLSHEETPARTSLIST(bool newFULLSHEETPARTSLIST)
{
    FULLSHEETPARTSLIST = newFULLSHEETPARTSLIST;
    finisheD.fullsheetpartslisT = true;
}

quint64 TemplateGenKiCAD_7::getNUMLINESFULLSHEETPARTSLIST() const
{
    return NUMLINESFULLSHEETPARTSLIST;
}

void TemplateGenKiCAD_7::setNUMLINESFULLSHEETPARTSLIST(quint64 newNUMLINESFULLSHEETPARTSLIST)
{
    NUMLINESFULLSHEETPARTSLIST = newNUMLINESFULLSHEETPARTSLIST;
    finisheD.numlinesfullsheetpartslisT = true;
}

const QMap<QString, TitelblockField> &TemplateGenKiCAD_7::getFULLSHEETPARTSLISTFIELDS() const
{
    return FULLSHEETPARTSLISTFIELDS;
}

void TemplateGenKiCAD_7::setFULLSHEETPARTSLISTFIELDS(const QMap<QString, TitelblockField> &newFULLSHEETPARTSLISTFIELDS)
{
